...
const contracts: Record<string, RouteContract> = {
  ...
  "app/api/snt-admin/social/analytics/sync/tiktok/route.ts": {
    methods: ["POST"], identity: /getAdminIdentity\(\)/, authorization: /"social:read"/,
    validation: { file: "app/api/snt-admin/social/analytics/sync/tiktok/route.ts", pattern: /isSameOriginAdminMutation\(request\.url, request\.headers\.get\("origin"\)\)/, exception: "No request body; provider and UAT database are server-owned." },
  },
