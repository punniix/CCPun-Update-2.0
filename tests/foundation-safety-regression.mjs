...
expect('production admin uses guarded article lifecycle', studioPolicy.includes('protectProductionArticleLifecycleActions') && studioPolicy.includes('ลบฉบับร่าง') && studioPolicy.includes('นำออกจากเว็บไซต์'));
expect('published URLs remain protected from permanent delete', studioPolicy.includes('if (hasPublicationHistory) return null;'));
