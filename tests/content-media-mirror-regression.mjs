...
const fullBackupWorkflow = await readFile(new URL('../.github/workflows/backup-sanity-production.yml', import.meta.url), 'utf8');
...
assert.match(builder, /rootRelativePath: posixRelative\(root, outputPath\)/);
assert.match(builder, /safeSegment\(role\).*safeSegment\(key \|\| 'asset'\).*originalName/);
assert.match(builder, /Featured image: \$\{featuredImagePath\}/);
...
assert.doesNotMatch(builder, /redirect:\s*['"]follow['"]/);
...
assert.match(sync, /Refusing symlink in mirror/);
assert.match(sync, /Drive mirror collision/);
assert.match(sync, /Drive mirror size verification failed/);
...
assert.match(fullBackupWorkflow, /cron:\s*'0 19 \* \* \*'/);
assert.doesNotMatch(fullBackupWorkflow, /build-content-media-mirror|sync-content-media-mirror/, 'Full disaster backup must stay independent from mirror failures');
