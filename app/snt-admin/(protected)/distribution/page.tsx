    ...
    <div>
      ...
      <section className="mt-7 grid gap-3 sm:grid-cols-3">
        ...
        <article className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
          ...
          <div className="mt-2 font-semibold">{runtime.databaseConfigured ? "ตั้งค่า Secret แล้ว" : "รอผูก Neon Secret"}</div>
        </article>
        <article className="rounded-2xl border border-white/10 bg-white/[0.035] p-4">
      ...
      <section className="mt-6 rounded-3xl border border-[#e0c985]/20 bg-[#e0c985]/[0.05] p-5">
        ...
        <div className="mt-2 text-sm text-white/60">{snapshot.variants.length} channel variants · ผ่านขั้นอนุมัติของ fixture แล้ว</div>
      </section>
      ...
      <section role="note" className="mt-6 rounded-3xl border border-amber-200/20 bg-amber-200/[0.05] p-5 text-sm leading-6 text-amber-50/80">
        ขอบเขต Phase 1: ตรวจ schema, state machine, idempotency และ audit contract เท่านั้น การเชื่อม OAuth, media storage, webhook และคำสั่ง publish จะพัฒนาใน phase ที่ได้รับอนุมัติภายหลัง
      </section>
    </div>
  );
}
