...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const canonical = article.canonical ?? `https://ccpun.com/blog/${article.slug}/`;
  const isDraft = article.status !== "published";
  const noindex = isDraft || article.noindex === true;
  ...
  return {
    ...
    openGraph: {
      ...
      ...(article.featuredImage ? { images: [{ url: article.featuredImage.src, alt: article.featuredImage.alt }] } : {}),
    },
  };
}
...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const isDraft = article.status !== "published";
  const schema = buildArticleSchemaGraph(article);
      ...
      <main id="main-content" className="min-h-screen bg-background pt-20">
        <article className="pb-24">
          ...
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            ...
            <p className="mb-8 mt-5 text-base leading-relaxed text-muted-foreground">{article.excerpt}</p>
