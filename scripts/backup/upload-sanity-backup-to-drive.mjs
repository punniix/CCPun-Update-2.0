...
async function uploadMultipart({ token, folderId, filePath, fileName, mimeType }) {
  ...
  const response = await fetch(
    'https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&supportsAllDrives=true&fields=id,name,size,webViewLink,md5Checksum',
    {
      method: 'POST',
      headers: {
  ...
  if (Number(result.size) !== buffer.length) {
    throw new Error(`Google Drive upload size mismatch for ${fileName}`);
  }
  return result;
}
...
async function main() {
  ...
  await appendSnapshotRow({
    ...
    values: [
      ...
      'Included + verified by Sanity CLI',
