...
const projectId = process.env.SANITY_PROJECT_ID?.trim() || 'kyfxgjnq';
...
if (projectId !== 'kyfxgjnq' || dataset !== 'production') throw new Error('Mirror is pinned to kyfxgjnq/production');

const client = createClient({ projectId, dataset, token, apiVersion: '2026-08-24', useCdn: false, perspective: 'raw' });
...
const query = `*[_type == "article" && defined(slug.current)] | order(_id asc){
  ...
  body[]{
    ...
    "media": select(
      _type == "imageWithAlt" => {"kind":"sanity","url":asset->url,"assetId":asset->_id,"fileName":asset->originalFilename,"mimeType":asset->mimeType,"size":asset->size,"sha1":asset->sha1hash,"width":asset->metadata.dimensions.width,"height":asset->metadata.dimensions.height},
      _type == "migratedImage" => {"kind":"migrated","url":src,"fileName":src,"width":width,"height":height},
    ...
    images[]{_key,alt,caption,credit,"media":{"kind":"sanity","url":asset->url,"assetId":asset->_id,"fileName":asset->originalFilename,"mimeType":asset->mimeType,"size":asset->size,"sha1":asset->sha1hash,"width":asset->metadata.dimensions.width,"height":asset->metadata.dimensions.height}}
  }
}`;
...
async function fetchBytes(url) {
  ...
  if (length > MAX_MEDIA_BYTES) throw new Error(`Media exceeds ${MAX_MEDIA_BYTES} bytes: ${url}`);
  const buffer = Buffer.from(await response.arrayBuffer());
  if (!buffer.length || buffer.length > MAX_MEDIA_BYTES) throw new Error(`Invalid media payload: ${url}`);
  return buffer;
}

async function mirrorMedia({ article, key, role, media }) {
  if (!media?.url) return null;
  const status = statusOf(article);
  const slug = safeSegment(article.slug);
  const fallbackName = `${role}-${safeSegment(key || 'asset')}`;
  let originalName = path.basename(String(media.fileName || media.url).split('?')[0]) || fallbackName;
  ...
  const fileName = `${safeSegment(role)}-${originalName}`;
  const outputPath = path.join(outputDir, fileName);
  let buffer;
  if (String(media.url).startsWith('/')) {
    const source = path.resolve('public', String(media.url).replace(/^\/+/, ''));
    const info = await stat(source);
    if (!info.isFile() || info.size > MAX_MEDIA_BYTES) throw new Error(`Invalid local migrated media: ${media.url}`);
...
const manifest = { schemaVersion: 1, createdAt: new Date().toISOString(), sanity: { projectId, dataset }, articles: [], media: [] };
...
console.log(JSON.stringify({ articles: manifest.articles.length, media: manifest.media.length, contentRoot, mediaRoot }, null, 2));
