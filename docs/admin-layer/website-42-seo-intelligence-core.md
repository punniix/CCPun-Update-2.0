...
GSC/GA4 observations are not assembled into detector input yet. The current detector contract also needs editorial intent, indexability, business-value, seasonality and last-change evidence, while the approved GA4 outcome-event registry and landing-page normalization contract are not settled. Keeping those joins out of this batch avoids guessing analytics or URL ownership.

This batch does not call Ubersuggest or a paid provider; persist observations or opportunities; generate AI proposals; mutate Sanity; apply Draft changes; publish content; or change URL, canonical, redirect, sitemap, robots, analytics or consent contracts. Search appearance remains deferred because Google documents it as a separate two-step query. Persistent sync runs, observation assembly and the opportunity lifecycle remain separate later batches.
